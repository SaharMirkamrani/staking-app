...
describe("🚩 Challenge 1: 🥩 Decentralized Staking App", function () {
  ...
  describe("Staker", function () {
    ...
    it("Should deploy ExampleExternalContract", async function () {
      ...
      console.log(
        "     🛰  exampleExternalContract contract deployed on",
    ...
    describe("🥩 Stake!", function () {
      ...
      it("Should redeploy Staker, stake, not get enough, and withdraw", async function () {
        ...
        const Staker = await ethers.getContractFactory(
          redeployedContractArtifact
        );
        stakerContract = await Staker.deploy(exampleExternalContract.address);
        ...
        const stakeResult = await stakerContract
          ...
          .stake({ value: ethers.utils.parseEther("0.00001") });
        console.log("\t", " 🏷  stakeResult: ", stakeResult.hash);
        ...
        const canWithdraw = await stakerContract.openForWithdraw();
        console.log("\t", " 🥁 openForWithdraw should be true: ", true);
        ...
        // expect(endingBalance.toNumber()).to.equal(
        //   startingBalance.add(ethers.utils.parseEther("0.00001")).sub(gasCost.toNumber())
        // );
      });
    });
  });
